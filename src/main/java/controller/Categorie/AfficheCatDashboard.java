...
import java.util.Optional;
    ...
    public class AfficheCatDashboard {
    ...
    public void initialize() {
        ...
        actionCol.setCellFactory(param -> new TableCell<>() {
            ...
            private final Button btnVoirOeuvres = new Button("Voir Œuvres"); // New button
            private final HBox box = new HBox(10, btnModifier, btnSupprimer, btnVoirOeuvres);

            {
                // Style des boutons
                btnModifier.getStyleClass().add("btn-modifier");
                btnSupprimer.getStyleClass().add("btn-supprimer");
                btnVoirOeuvres.getStyleClass().add("btn-voir"); // Add CSS class
    ...
    private void modifierCategorie(CategorieOeuvre categorie) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/Dashboard/ModifierCategorie.fxml"));
    ...
    }// Dans la méthode voirOeuvres, remplacer le chargement par :

        private void voirOeuvres(CategorieOeuvre categorie) {
            try {
                String fxmlPath = System.getProperty("user.dir") + "/src/main/resources/Dashboard/oeuvre-by-categorie.fxml";
                FXMLLoader loader = new FXMLLoader(new File(fxmlPath).toURI().toURL());

                Parent root = loader.load();

                // Passer la catégorie sélectionnée au contrôleur
                OeuvreByCategorieController controller = loader.getController();
                controller.setSelectedCategory(categorie);

                Stage stage = new Stage();
                stage.setScene(new Scene(root));
                stage.setTitle("Œuvres - " + categorie.getNomCategorie());
                stage.show();
            } catch (IOException e) {
                afficherErreur("Erreur", "Impossible d'afficher les œuvres: " + e.getMessage());
            }
        }

        @FXML
        private void redirectToModifyCategoryDash(ActionEvent event) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/Dashboard/ModifierCategorie.fxml"));
                Parent root = loader.load();
